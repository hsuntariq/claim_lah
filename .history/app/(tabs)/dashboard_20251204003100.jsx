...
export default function ExpenseDashboard({ onLogout }) {
  ...
  // FIXED: Prepare data for expo-charts (array of { label: string, value: number })
  const weeklyData = getWeeklyData().map((item) => ({
    label: item.day,  // e.g., "Mon"
    value: item.amount,  // e.g., 25.50
  }));

  // FIXED: Prepare data for expo-charts PieChart (array of { label: string, value: number })
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        >
          ...
          {/* Weekly Chart - FIXED for expo-charts */}
          <View style={styles.chartContainer}>
            ...
            <View style={styles.chartWrapper}>
              {weeklyData.some((d) => d.value > 0) ? (
                <LineChart
                  data={weeklyData}  // Now a simple array: [{ label: "Mon", value: 25.50 }, ...]
                  width={width - 60}
                  height={220}
                  lineColor="#27AE60"  // Line color
                  accentColor="#2ECC71"  // Dots/fill color
                  labelColor="#7F8C8D"  // X-axis labels
                  axisColor="#BDC3C7"  // Axes color
                  backgroundColor="#F8F9FA"
                  title="Weekly Spending"
                  subtitle="Trend over days"
                  formatValue={(value) => `$${value.toFixed(0)}`}  // Y-axis formatter
                  formatLabel={(index) => weeklyData[index]?.label || ""}  // X-axis formatter
                  showGrid={true}
                  animate={true}
                  showDots={true}
                  showArea={false}  // Set to true for area chart
          ...
          {/* Category Breakdown - FIXED for expo-charts */}
          <View style={styles.chartContainer}>
            <View style={styles.chartHeader}>
            ...
            <View style={styles.pieChartWrapper}>
              {pieData.length > 0 ? (
                <>
                  <PieChart
                    data={pieData}  // Now a simple array: [{ label: "Food", value: 150 }, ...]
                    width={width - 60}
                    height={200}
                    primaryColor="#27AE60"  // Main slice color (or use getSliceColor for custom)
                    ...
                    animate={true}
                    showValues={true}
                    getSliceColor={(index) => categories[index]?.color || "#27AE60"}  // Custom colors from your categories
                  />
