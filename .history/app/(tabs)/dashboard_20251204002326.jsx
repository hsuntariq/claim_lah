...
export default function ExpenseDashboard({ onLogout }) {
  ...
  const LineChart = ({ data, width: chartWidth, height: chartHeight }) => {
    const maxValue = Math.max(...data.map((item) => item.amount), 1);
    const barWidth = (chartWidth - 40) / data.length;
    const barHeightScale = (chartHeight - 60) / maxValue;
    ...
    return (
      <Svg width={chartWidth} height={chartHeight}>
        ...
        {[0, 0.25, 0.5, 0.75, 1].map((ratio, index) => (
          <Path
            key={`grid-${index}`}
            d={`M 20 ${20 + ratio * (chartHeight - 60)} L ${chartWidth - 20} ${
              20 + ratio * (chartHeight - 60)
            }`}
            stroke="#E0E0E0"
            strokeWidth="1"
            strokeDasharray="4 4"
        ...
        {/* Bars */}
        {data.map((item, index) => {
          const barHeight = item.amount * barHeightScale;
        ...
        {/* Line connecting the tops of bars */}
        <Path
          d={`M ${data
            .map((item, index) => {
              const barHeight = item.amount * barHeightScale;
              const x = 30 + index * barWidth + (barWidth - 10) / 2;
              const y = chartHeight - 40 - barHeight;
  ...
  const PieChart = ({ data, width: chartWidth, height: chartHeight }) => {
    ...
    return (
      <Svg width={chartWidth} height={chartHeight}>
        ...
        {/* Labels */}
        {slices.map((slice, index) => {
          if (slice.percentage < 0.05) return null; // Skip very small slices

          const labelRad = ((slice.labelAngle - 90) * Math.PI) / 180;
        ...
        >
          ...
          <View style={styles.chartContainer}>
            ...
            <View style={styles.chartWrapper}>
              <LineChart data={weeklyData} width={width - 60} height={200} />
            </View>
          </View>
          ...
          <View style={styles.chartContainer}>
            ...
            <View style={styles.pieChartWrapper}>
              {pieData.length > 0 ? (
                <>
                  <PieChart data={pieData} width={width - 60} height={200} />
