...
export default function ExpenseDashboard({ onLogout }) {
  ...
  // Add this helper anywhere in your file
  const safeNumber = (n) =>
    isNaN(n) || n === null || n === undefined ? 0 : Number(n.toFixed(2));
  ...
  // SAFELY prepare weekly data — NO NaN allowed!
  const weeklyData = (() => {
    const now = new Date();
    const startOfWeek = new Date(now);
    ...
    for (let i = 0; i < 7; i++) {
      ...
      const dayExpenses = expenses.filter((e) => {
        ...
        return ed.toDateString() === currentDay.toDateString();
      });
      ...
      result.push({
        label: days[i],
        value: isNaN(total) ? 0 : Number(total.toFixed(2)), // ← CRITICAL FIX
      });
    }
  ...
  const pieData = categories
    ...
    .map((cat) => ({
      label: cat.name,
      value: Number(cat.value.toFixed(2)), // ← Force valid number
    }));
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        >
          ...
          {/* Weekly Chart - 100% Safe from NaN crash */}
          <View style={styles.chartContainer}>
            ...
            {weeklyData.some((d) => d.value > 0) ? (
              <LineChart
                ...
                animate
                formatValue={(v) => `$${v}`}
                // This is the KEY prop to prevent crashes:
                yAxisInterval={1}
                fromZero
                // Optional: filter out zero days for cleaner look
                segments={4}
              />
            ) : (
              <View style={styles.emptyChart}>
                <Ionicons name="trending-up" size={48} color="#D5D5D5" />
                <Text style={styles.emptyChartText}>No spending this week</Text>
              </View>
            )}
          </View>
